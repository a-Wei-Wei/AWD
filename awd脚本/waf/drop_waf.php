...
function detect_upload(){
    $switch_upload_check = 1;
    $attack_flag = 0;
    if ($switch_upload_check === 1) {
        $white_list = ['image/png','image/jpg','image/gif','image/jpeg'];
    	foreach ($_FILES as $key => $value) {
            if($_FILES[$key]['size']>1){
                //判断 后缀名是否符合要求
                $file_name = $_FILES[$key]['name'];
                $file_array = explode(".", $file_name);
                $file_post_name = $file_array[count($file_array) - 1 < 0 ? 0 : count($file_array)-1];
                $white_str = implode(" ",$white_list);
                $post_name_result = strpos($white_str, $file_post_name);
                //判断 content-type 是否符合要求
                $content_type_name = $_FILES[$key]['type'];
                $content_type_check = strpos($white_str, $content_type_name);
                if ($content_type_check === false || $post_name_result === false) {
                    var_dump($_FILES);
                    echo("don't hack");
                    $attack_flag = 1;
                }
    			...
    			#move_uploaded_file($_FILES[$key]["tmp_name"],'/tmp/uploadFiles/'.$_FILES[$key]["name"]);
    		}
        }
    }
}
...
function write_access_log_probably() { 
	...
	$ffff = fopen($this->path.'all_requests.txt', 'a'); //日志路径 
    fwrite($ffff, $raw);  
    fclose($ffff);
}
...
function write_access_logs_detailed(){
    ...
    $ffff = fopen($this->path.'all_requests_detail.txt', 'a'); //日志路径 
...
function write_attack_log($alert){
    ...
    $ffff = fopen($this->path.'attack_detected_log.txt', 'a'); //日志路径 
    fwrite($ffff, $data);  
    fclose($ffff);
    if($alert == 'GETFLAG'){
